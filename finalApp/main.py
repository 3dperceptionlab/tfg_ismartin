...
if len(sys.argv)!=2:
	print("ERROR: Invalid number of arguments\nUsage: python main.py <video_name_to_analyze>")
	exit(-1)

# th architecture to use in stage recognition
arch = 'resnet50'
# load the pre-trained weights
model_file = '%s_places365.pth.tar' % arch

# Cargamos el modelo
modelStage = models.__dict__[arch](num_classes=365)
checkpoint = torch.load("../models/" + model_file, map_location=lambda storage, loc: storage)
state_dict = {str.replace(k,'module.',''): v for k,v in checkpoint['state_dict'].items()}
modelStage.load_state_dict(state_dict)
modelStage.eval()
...
with open(file_name) as class_file:
    for line in class_file:
        stage_classes.append(line.strip().split(' ')[0][3:])
...
def draw_boxes(filename, v_boxes, v_labels, v_scores):
	...
	pyplot.show()
	# save the plot
	pyplot.savefig('outputs/result.png')
...
import glob
#name = "/mnt/md1/datasets/toyota_smarthome/rgb/mp4/Uselaptop_p15_r01_v10_c01.mp4"
name = sys.argv[1]
...
while success:
	...
	if count%1000==0:
		...
		v_boxes, v_labels, v_scores, v_index = get_boxes(boxes, labels, class_threshold)	#OPTIMIZAR
		...
		######################## DETECCION DE ESTANCIAS ###########################
		#print(type(imageOrig))	#Deberia ser pil
		#print(imageOrig.shape)
		#image = Image.fromarray(np.uint8(cm.gist_earth(imageOrig)*255))
		#image = Image.fromarray(np.uint8(imageOrig*255))
		imageOrig = imageOrig[:,:,::-1]
		image = Image.fromarray(np.uint8(imageOrig))
		#image = PIL.ImageOps.invert(image)
		
		#print(type(image))	#Deberia ser pil
		#image.save('imageToStageRec.jpg')
		#cv2.imwrite('imageOrig.jpg', imageOrig)
		input_img = V(centre_crop(image).unsqueeze(0))

		# forward pass
		logit = modelStage.forward(input_img)
...
possibleActions=set()
kitchen = stage_classes.index('kitchen')
dining_room = stage_classes.index('dining_room')
television_room = stage_classes.index('television_room')

for stg in idx[:5]:
	possibleStage.add(stage_classes[stg])

if kitchen in idx[:5]:
		actionsStage.add('cook')
		actionsStage.add('cutbread')
		actionsStage.add('maketea')
		actionsStage.add('makecoffee')
		possibleActions.add('cook')
		possibleActions.add('cutbread')
		possibleActions.add('maketea')
		possibleActions.add('makecoffee')
if dining_room in idx[:5]:
		actionsStage.add('eat')
		possibleActions.add('eat')
if television_room in idx[:5]:
		actionsStage.add('watchtv') 
		possibleActions.add('watchtv') 
...
print("Possible stages: ")
for stg in possibleStage:
	print(stg)
print()

print("Actions from stages:")
for act in actionsStage:
	print(act)
print()

print('Total possible actions:')
